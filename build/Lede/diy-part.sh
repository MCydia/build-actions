...
# 旁路由选项
export Gateway_Settings="10.10.10.251"      # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
...
# 更换固件内核
export Replace_Kernel="5.15.137"             # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
...
sed -i 's/"TTYD 终端"/"TTYD"/g' `egrep "终端" -rl ./`
sed -i 's/"USB 打印服务器"/"打印服务"/g' `egrep "打印服务" -rl ./`
sed -i 's/"Web 管理"/"Web管理"/g' `egrep "管理" -rl ./`
